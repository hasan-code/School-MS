...
const nameInput = document.getElementById("receiver");
const suggestionsModal = document.getElementById("name-suggestions-modal");
const nameLists = document.querySelectorAll(".names");

nameInput.addEventListener("input", function () {
  const inputText = nameInput.value.toLowerCase();

  for (let i = 0; i < nameLists.length; i++) {
    const name = nameLists[i].textContent.toLowerCase();

    if (name.includes(inputText)) {
      suggestionsModal.style.display = "block";
      nameLists[i].style.display = "";
    } else {
      suggestionsModal.style.display = "none";
      nameLists[i].style.display = "none";
    }
  }
});

// Add click event listener to each filtered name item
for (let i = 0; i < nameLists.length; i++) {
  nameLists[i].addEventListener("click", function () {
    const selectedName = nameLists[i].textContent;
